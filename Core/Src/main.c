...
int main(void)
{
  ...
  // =======================================================
  // 🌟 强行配置 IMU 为 6轴模式 (屏蔽地磁场畸变干扰)
  // =======================================================
  printf("🔄 正在通过代码强制配置 IMU 模块为 6轴模式...\r\n");

  // 依据官方协议组装的 16进制指令：切换为 6 轴算法
  uint8_t imu_config_cmd[] = {0x7E, 0x23, 0x07, 0x61, 0x06, 0x5F, 0x6E}; 
  
  // 通过 UART8 发给 IMU 模块
  HAL_UART_Transmit(&huart8, imu_config_cmd, sizeof(imu_config_cmd), 100);
  
  // 给模块 200ms 的算法切换和重启时间
  HAL_Delay(200); 
